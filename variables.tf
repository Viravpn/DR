...
variable "name" {
  description = "FGT-VM instance name."
  type        = string
}
...
variable "username" {
  description = "Initial administrative username to use for FGT-VM. Mind the [Azure-imposed restrictions](https://docs.microsoft.com/en-us/azure/virtual-machines/linux/faq#what-are-the-username-requirements-when-creating-a-vm)."
  type        = string
}
...
variable "sku" {
  description = "list available with `az vm image list -o table --all --publisher paloaltonetworks`"
  default     = "fortinet_fg-vm"
  type        = string
}

variable "fg_version" {
 description = "list available for a default `img_offer` with `az vm image list -o table --publisher paloaltonetworks --offer vmseries-flex --all`"
 type        = string
}
